...
function drawFloor() {
    ...
    for (let c = 0; c < density; c++) {
        for (let r = 0; r < density; r++) {
            let frame = floor(
                noise(r * 2 + c * 3 * 100) * imgs_floorTexture.length
            )
            let x =
                (width / density) * (c + 0.5) +
                ((noise((r * 13 + c * 17) * 10) - 0.5) * width) / density
            let y =
                (height / density) * (r + 0.5) +
                ((noise((r * 23 + c * 19) * 40) - 0.5) * width) / density
            image(
                imgs_floorTexture[frame],
                x,
                y,
                imgs_floorTexture[0].width * scale,
                imgs_floorTexture[0].height * scale
            )
        }
    }
}
...
function drawTimer() {
    ...
    text(
        'time: ' + floor(time) + nf(time - floor(time), 0, 2).substring(1),
        u * 5,
        height - u * 5
    )
}
