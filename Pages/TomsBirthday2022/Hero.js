class Hero {
	...
	draw() {
		...
		if (this.dashing && !this.frozen) {
			this.drawTrail();
			image(img_hero_dash, this.pos.x, this.pos.y, img_hero_dash.width * scalar, img_hero_dash.height * scalar);
		}
		else {
			let idleFrame= (floor(millis() / 200) % 2);
			image(imgs_hero_idles[idleFrame], this.pos.x, this.pos.y, img_hero_idle_1.width * scalar, img_hero_idle_1.height * scalar);
		}
	}
