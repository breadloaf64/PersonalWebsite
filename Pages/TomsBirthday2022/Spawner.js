class Spawner {
	...
	trySpawnFastSkeleton() {
		let period = 1 / this.spawnFreqFastSkeleton[min(this.phase, this.spawnFreqFastSkeleton.length - 1)];
		if (millis() - this.timeLastFastSkeleton > period * 1000) {
			enemies.push(new Skeleton(this.makeSpawnLocation(), u * 0.025, 0.65));
			this.timeLastFastSkeleton = millis();
		}
	}
	
	trySpawnSkeleton() {
		let period = 1 / this.spawnFreqSkeleton[min(this.phase, this.spawnFreqSkeleton.length - 1)];
	...
	trySpawnSpider() {
		let period = 1 / this.spawnFreqSpider[min(this.phase, this.spawnFreqSpider.length - 1)];
	...
	makeSpawnLocation() {
		...
		if (side == 0) return createVector(-border, p); // left
		else if (side == 1) return createVector(width + border, p); // right
		else if (side == 2) return createVector(p, -border); // top
