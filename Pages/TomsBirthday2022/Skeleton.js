class Skeleton {
	...
	draw() {
		...
		image(img_skeleton, 0, 0, img_skeleton.width * scalar, img_skeleton.height * scalar);
		pop();
	}

	update() {
		if (!this.atCake) { // move to cake
			let toCake = cake.pos.copy().sub(this.pos);

			// move
			let moveDist = min(toCake.mag() - cake.eatRadius, this.moveSpeed * deltaTime);
			...
			if (millis() - this.lastBiteTime > this.biteCooldown) {
				...
				textPopups.push(new TextPopup(textPopupPos, random(["nom", "om"]), u * 2, 700));
			}
		}
