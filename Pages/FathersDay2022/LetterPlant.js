...
class LetterPlant {
	constructor(x, y, fci) {
		...
		this.waveAmount = 0.1;
		this.angle = 0;
		this.flower = new Flower(this.stemPosAtY(this.maxHeight), -this.maxHeight, fci);
	...
	drawStem() {
		...
		while (0 - stemY < this.height) {
			...
			line(this.stemPosAtY(stemY), stemY, this.stemPosAtY(nextStemY), nextStemY);
	...
	update() {
		...
		this.time++;
		this.angle = map(sin(this.time / 100 * this.waveFreq), -1, 1, -PI / 16 * this.waveAmount, PI / 16 * this.waveAmount);
		this.flower.tick();
	}
	...
	water(x) {
		if (abs(x - this.seedPos.x) < width * 0.1 && this.age <= this.maxAge) {
			this.age++;
			this.height = map(this.age, 0, this.maxAge, this.minHeight, this.maxHeight);
			...
			if (this.leaves.length < numLeaves) {
				...
				this.leaves.push(new Leaf(this.stemPosAtY(y), -y, (this.maxLeafSize - (numLeaves * width / 71)) * map(random(1), 0, 1, 0.8, 1.2), angle));
			}
		}
	}
