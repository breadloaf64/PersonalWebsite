...
function updateCurrentVoice() {
	...
	if (!paused) {
		...
		if (mouseIsPressed && prevMouseIsPressed) { // this is so that on touch screen, if the user puts their finger down, there won't be a line between the position and previous position.
			if (mouseX == prevMouseX && mouseY == prevMouseY || ml.x == mr.x) { // yeah, this double if is dodgy, I know. But I think it's more readable
				...
				for (let i = round(ml.x); i <= round(mr.x); i++) {
					let lerpMouseX = i;
					let lerpMouseY = round(ml.y + (mr.y - ml.y) * (i - ml.x) / (mr.x - ml.x));
					let t = map(lerpMouseX, 0, width, 0, 1, true);
					let v = map(lerpMouseY, height, 0, 0, 1, true);
					if (isNaN(v)) {
						print("i: " + i + ", ml: (" + ml.x + ", " + ml.y + "), mr: (" + mr.x + ", " + mr.y + ")");
					}
