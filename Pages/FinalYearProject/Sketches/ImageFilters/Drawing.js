...
function drawFilterName() {
  ...
  rect(
    10,
    height - txtSize - 20,
    textWidth(filterNames[filterSelection]) + 20,
    txtSize * 1.5,
  );
...
function drawCapture() {
  ...
  if (filterSelection == 0);
    ...
    img.filter(THRESHOLD, map(mouseY, height, 0, 0, 1), true);
  else if (filterSelection == 2) img.filter(GRAY);
  else if (filterSelection == 3) img.filter(INVERT);
  else if (filterSelection == 4)
    img.filter(POSTERIZE, round(map(mouseY, height, 0, 2, 10, true)));
  else if (filterSelection == 5)
    img.filter(BLUR, round(map(mouseY, height, 0, 1, 4, true)));
