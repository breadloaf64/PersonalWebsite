...
function makeSequence(voiceCount, source, mapsCentresToVoices, colsCentres) {
  ...
  for (let i = 0; i < voiceCount; i++) {
    // loop through the voices that need to be added to the sequence
    let currentVoice = new Voice();

    for (let col = 0; col < floor(source.width) - 1; col++) {
      ...
      if (
        currMapCentresToVoices.includes(i) &&
        nextMapCentresToVoices.includes(i)
      ) {
        // if one of the col centres is mapped to the current voice
        let h1 =
          currColCentres[
            currMapCentresToVoices.findIndex((element) => element == i)
          ]; // height on image of current col for this voice
        ...
        let h2 =
          nextColCentres[
            nextMapCentresToVoices.findIndex((element) => element == i)
          ]; // height on image of next col for this voice
  ...
  sequences[currentSequenceIndex].setMaxSimultaneousVoices(
    getMaxSimultaneousVoices(colsCentres),
  );
...
function getMapsCentresToVoices(colsCentres) {
  ...
  for (let i = 1; i < colsCentres.length; i++) {
    ...
    if (prevColCentres.length < currColCentres.length) {
      // loop over centres of current previous column and look ahead
      ...
      for (let j = 0; j < prevColCentres.length; j++) {
        ...
        let nearestCurrCentre = getNearestArrValue(
          currPrevCentre,
          currColCentres,
          indicesMatched,
        ); // the nearest centre in the current column

        let indexOfNearest = currColCentres.findIndex(
          (element) => element == nearestCurrCentre,
        );
      ...
      for (let j = 0; j < currColCentres.length; j++) {
        // loop over centres of current column and look back
        let currCentre = currColCentres[j];
        let nearestPrevCentre = getNearestArrValue(
          currCentre,
          prevColCentres,
          indicesMatched,
        );

        let indexOfNearest = prevColCentres.findIndex(
          (element) => element == nearestPrevCentre,
        );
...
function getNearestArrValue(target, arr, ignoreIndices) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (!ignoreIndices.includes(i)) {
      if (
        abs(arr[i] - target) < abs(nearestVal - target) ||
        isNaN(nearestVal)
      ) {
        nearestVal = arr[i];
      }
