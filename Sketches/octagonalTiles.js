...
const stagePeriods = [3 * rotPeriod16th, 4 * rotPeriod16th, 3 * rotPeriod16th, 5 * rotPeriod16th]
...
function update() {
	...
	if (abs(counter - 0) < transitionLength) {
		evadeMouseAmount = sigmoidMap(abs(counter - 0), 0, transitionLength, 0, evadeMouseAmountAbs);
	} else if (abs(counter - stagePeriods[stage]) < transitionLength) {
		evadeMouseAmount = sigmoidMap(abs(counter - stagePeriods[stage]), 0, transitionLength, 0, evadeMouseAmountAbs);
	} else {
...
function pattern_sq(x, y, l, n) {
	...
	for (let i = 0; i < n + 1; i++) {
		for (let j = 0; j < n + 1; j++) {
			let pos = createVector(x + i * boundingBoxLength, y + j * boundingBoxLength);
...
function pattern_star(x, y, l, n) {
	...
	let longR = boundingBoxLength / 2 -
		tan(radians(135) / 2) *
		(circumradius - boundingBoxLength / 2);
